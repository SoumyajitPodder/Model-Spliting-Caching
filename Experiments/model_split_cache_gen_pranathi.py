...
def _object_nbytes(obj, seen=None):
    if seen is None:
        seen = set()
    ...
    if isinstance(obj, (list, tuple, set)):
        return sum(_object_nbytes(item, seen) for item in obj)
    if isinstance(obj, dict):
        return sum(_object_nbytes(value, seen) for value in obj.values())
    if hasattr(obj, "__dict__"):
        return _object_nbytes(vars(obj), seen)
    return 0

def cache_size_mb(cache_obj):
    return _object_nbytes(cache_obj) / (1024 * 1024)
...
def save_handoff_package(hidden, position_embeddings=None, position_ids=None, save_dir="./handoff"):
...
def perform_split_generation(tokens_to_generate):
    ...
    for _ in range(tokens_to_generate):
        ...
        cache_a_mb = cache_size_mb(cache_a)
        peak_cache_a_mb = max(peak_cache_a_mb, cache_a_mb)
        # perform split 1
        ...
        cache_b_mb = cache_size_mb(cache_b)
        peak_cache_b_mb = max(peak_cache_b_mb, cache_b_mb)
    ...
    h2.remove()
    print("\n========== CACHE SIZE METRICS ==========")
    print(f"Cache A current size: {cache_size_mb(cache_a):.2f} MB")
    print(f"Cache A peak size:    {peak_cache_a_mb:.2f} MB")
    print(f"Cache B current size: {cache_size_mb(cache_b):.2f} MB")
    print(f"Cache B peak size:    {peak_cache_b_mb:.2f} MB")
